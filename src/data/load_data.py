...
def process_wy2f(file_path: str, time_correction: datetime.timedelta):
    ...
    def date_parser(d):
        datetime.datetime(int(d[0:4]), int(d[4:6]),
    ...
    D = pd.read_fwf(file_path, colspecs=WY2_cols,
                    ...
                    converters={'T_flag': lambda tf: Tflags[tf]})
    D.loc[:, 'T'] = D.loc[:, 'T'] / 10
...
def wy2_to_hdf(metadata_row: pd.Series):
    ...
    D = process_wy2f(metadata_row['WBAN_file'],
                     metadata_row['time_correction'])
...
def main():
    ...
    hdf.create_group('/', TEMPERATURE_TS_ROOT)
    hdf.close()
